...
module.exports = {
    ...
    delete: async (req, res) => {
        ...
        const finance = await Finance.findOneAndDelete({ _id });
        if (finance) {
    ...
    updateSpent: async (req, res) => {
        const { _id } = req.params;
        const { label, isSpent, amount, date } = req.body;
        const user = req.user;
        const finances = await Finance.findOne({ _id });
        const neg= Math.abs(amount)
        finances.label=label;
        finances.isSpent=true;
        finances.amount=neg;
        finances.date=date;
        finances.user=user;
        await finances.save();
