...
module.exports = {
    create: async (req, res) => {
        ...
        const myTerrain = await Terrain.findOne({ name: terrain.name, user: req.user })
    ...
    update: async (req, res) => {
        ...
        const { Name, StartTime, EndTime, terrain, num, frais } = req.body;
        let myTerrain=[]; 
        typeof terrain === 'object' ?  myTerrain = await Terrain.findOne({ name: terrain.name, user: req.user }) : myTerrain = await Terrain.findOne({ name: terrain, user: req.user })
       const isThereAReservation = await Reservation.findOne({
        _id: { $ne: _id },
        terrain: myTerrain,
        $or: [
